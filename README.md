...
| VTT 미리보기 | 선택한 SRT의 변환 결과와 자막 개수를 저장 전에 확인합니다. |
| VTT 자막 뷰어 | 미리보기 탭에서 기존 VTT 파일을 직접 열어 내용을 확인합니다. |
...
| 출력 위치 선택 | 원본 폴더, `VTT` 하위 폴더 또는 사용자가 지정한 폴더에 저장합니다. |
...
3. 파일을 선택하고 `VTT 미리보기` 탭에서 변환 결과를 확인합니다.
4. 저장 위치, 덮어쓰기 여부, 시간 보정값을 설정합니다.
5. 다른 위치에 저장하려면 `직접 폴더 지정`을 선택하고 폴더를 고릅니다.
6. `VTT 변환 시작`을 누릅니다.
7. 변환이 끝나면 `결과 열기` 버튼으로 출력 폴더를 확인합니다.
